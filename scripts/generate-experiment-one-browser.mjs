...
try {
    ...
    for (const output of outputs) {
        ...
        for (const viewpoint of EXPERIMENT_ONE_VIEWPOINTS) {
            await capture(page, output, "stills", viewpoint.id, viewpoint.pose, 0, record.files);
        }
        if (output.includePaths) {
            for (const pathDocument of EXPERIMENT_ONE_PATHS) {
                for (const sample of pathDocument.samples) {
                    await capture(page, output, pathDocument.id, String(sample.sampleIndex).padStart(4, "0"), sample.pose, sample.captureTimeNs, record.files);
                }
            }
        }
        ...
        console.log(`${output.id}: ${Object.keys(record.files).length} PNGs`);
    }
} finally {
    await browser.close();
...
async function capture(page, output, group, sampleId, pose, captureTimeNs, fileHashes) {
