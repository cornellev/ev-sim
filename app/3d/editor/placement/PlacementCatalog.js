...
import { Cone } from "../../city/objects/Cone.js";
 ...
 */
export function createFusionObject(assetId, point, options = {}) {
    const position = point instanceof THREE.Vector3
        ? point.clone()
        : new THREE.Vector3(point.x, point.y ?? 0, point.z);

    if (assetId === "stop-sign") {
        return new StopSign(position, new Unit(5, Unit.Type.FOOT), options.dir ?? 1);
    }

    if (assetId === "one-way-sign") {
        return new OneWaySign(position, options.dir ?? 1);
    }
