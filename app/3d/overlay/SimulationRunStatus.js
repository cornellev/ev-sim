import { FaCircle, FaClock, FaDatabase, FaFileAlt, FaFlagCheckered } from "react-icons/fa";
...
export function SimulationRunStatus({ simState, runState, recordingState }) {
    ...
    return (
        ...
        >
            <div className="grid min-h-14 grid-cols-[minmax(170px,0.85fr)_minmax(220px,1.5fr)_110px_120px] items-center divide-x divide-zinc-800 overflow-hidden rounded-xl border border-zinc-700/80 bg-zinc-950/90 text-zinc-100 shadow-[0_16px_45px_rgba(0,0,0,0.38)] backdrop-blur-xl max-lg:grid-cols-[minmax(150px,1fr)_minmax(180px,1.2fr)_100px]">
                <div className="min-w-0 px-3.5 py-2">
                    ...
                    <p className="mt-0.5 truncate pl-4 text-[9px] text-zinc-500">{presentation.detail}</p>
                </div>
                ...
                <div className="min-w-0 px-3.5 py-2">
                    ...
                    {manifest ? (
                        <div className="mt-0.5 flex min-w-0 items-baseline gap-2">
                            ...
                            <p className="shrink-0 font-mono text-[9px] text-zinc-500">{manifest.id} · {String(resolved.resolvedHash || "unresolved").slice(0, 8)}</p>
                        </div>
                    ) : (
                        <div className="mt-0.5 flex min-w-0 items-baseline gap-2">
                            <p className="truncate text-[11px] font-semibold text-zinc-300">No manifest loaded</p>
                            <p className="truncate text-[9px] text-zinc-600">{runState?.selectedManifestId ? `${runState.selectedManifestId} selected` : "Use Config to launch one"}</p>
                        </div>
                    )}
                </div>
            ...
            <div className="mx-3 grid grid-cols-[1fr_auto] border-x border-b border-zinc-800 bg-zinc-950/85 px-3 py-1 text-[9px] text-zinc-500 max-sm:hidden">
