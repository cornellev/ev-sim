...
export function AssetCatalogInspector({ data, tab }) {
    ...
    return <div className="space-y-4 p-3 text-xs" data-asset-catalog-inspector>
        <header>
            <h3 className="text-sm font-medium text-zinc-100">{tab.name}</h3>
            <p className="text-zinc-400">Revision {status?.revision ?? tab.revision}{status?.dirty ? " · Unsaved" : ""}</p>
            <p className="font-mono text-[11px] text-zinc-300" data-asset-id={tab.id}>Asset ID {tab.id}</p>
        </header>
