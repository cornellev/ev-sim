...
import { Tire } from "../../city/objects/Tire";
...
export async function FI2(scene, data) {
    const roads = [];
    let baseOffset = null;
    let segStart = null;
    let segEnd = null;
    ...
    const curve = new THREE.CatmullRomCurve3([segStart, segStart.clone().lerp(segEnd, 0.5), segEnd], false, "catmullrom", 0.15);
    const tirePos = curve.getPointAt(0.44);
    tirePos.y = 0;
    const tire = new Tire(new THREE.Vector3(tirePos.x, tirePos.y, tirePos.z), 0.22, 0.055);
    data.objects().addObject(tire);
