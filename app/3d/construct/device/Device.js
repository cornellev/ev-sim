...
export default class Device {
    ...
    //todo: figure out a better way to do this, potentially hook function etc
    getOverlay({ disableOrbit, enableOrbit, ...props }) {
        const [settings, setSettings] = useState(this.settings);
        const [enabled, setEnabled] = useState(this.enabled);

        return (
            <div className="relative bg-[#2b2b2b] rounded-md border border-[#555555] text-white shadow-lg min-w-[260px] max-w-sm" onMouseDown={disableOrbit} onMouseUp={enableOrbit}>
                <div className="flex items-center justify-between px-3 py-2 bg-[#3a3a3a] rounded-t-md border-b border-[#555555]">
                    <p className="text-xs font-semibold tracking-wide select-none">{this.name}</p>
                </div>
    ...
    overlayConstructor() {
        return this.getOverlay.bind(this);
    }
}
