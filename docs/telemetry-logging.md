...
Replay loads the binary index and schemas, seeks from checkpoint state, applies exact updates to inspectors, and interpolates visual poses between samples. Replay also exposes a **Map** view for 2D route and trajectory inspection over recorded environment attachments. The **Logs** workspace is the recording library and evidence front door: folders, rename, tags, size, import, download, delete, searchable provenance (hashes, suite/result/case IDs, git, candidate models), and deep links to Config manifests, Experiment result/case/baseline review, Replay, and Analysis. Analysis shares the timeline cursor and supports local live data, another browser tab, or a backend log. Its **Autonomy** tab renders the same spatial map for `log:*` sources with capture-aligned diagnostics beside the map. Layout JSON is stored under `analysis:layout:v1` and can be exported and edited.
