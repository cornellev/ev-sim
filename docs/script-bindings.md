...
- All bound scripts share one **signal store**. Topic updates are bridged into `topics.<name>`, and each fixed step writes a `simulation` snapshot (`{ dt, time, step, frame }`), so signal blocks inside scripts observe live data. Integrator and PID `dt` ports are ordinary program inputs; map them with `source: "sim", key: "dt"`, or wire `SimulationClockBlock.dt`. `SimulationClockBlock` reads kernel leafs `simulation.time` / `.step` / `.status` when present and prefers the bindings blob `dt` over `simulation.fixedDt`. Vehicle adapters read `vehicles.{id}.pose`, `.velocity`, and `.steeringAngle` (not `vehicles.{id}.steering`).
