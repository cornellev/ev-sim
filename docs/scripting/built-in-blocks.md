...
- `simulator`: path-configured vehicle, device, simulation, scenario, and object snapshots, plus typed adapters (`VehicleStateBlock`, `DeviceStateBlock`, `SimulationClockBlock`, `ScenarioStatusBlock`), Sample Road, Spawn Prop, and Scatter Features. Spawn Prop and Scatter Features upsert onto the episode overlay (`episode:*` ids). They do not write the environment document. Bind scatter graphs to `episode-reset` (`phase: start` on Play / prepare, `phase: stop` on Stop).
...
`FrameAlongPathBlock`, `SampleRoadBlock`, `SpawnPropBlock`, and `ScatterFeaturesBlock` live in `app/scripting/units/world/WorldBlocks.block.js`. Path math is `app/roads/PathFrame.js`. Spawn writes `SimulationRuntimeContext.episodeOverlay`, which the kernel applies to physics and CPU LiDAR on reset. Overlay ids use the `episode:` prefix. Missing overlay host throws `Spawn Prop requires an episode overlay (bind the script to episode-reset).` Scatter requires exactly one of `route` or `edgeId`. `worldHash` and frozen `lidarGeometryHash` stay unchanged.
