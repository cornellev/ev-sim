...
Triggers (`topic`, `fixed-update`, `signal-update`, `timer`, `simulation-timer`, `episode-reset`) are the run modes. `episode-reset` accepts `phase`: `start` (Play / prepare) or `stop` (Stop). Input/output labels must match the script artifact's `interface`. Spawn Prop / Scatter Features graphs belong on `episode-reset`.
