...
- Directed A\* `verification.polyline` algorithm version 5 (v1 roads) or 7 (geometry-v2 roads; v6 proofs are stale after ED-05) is the hashed travel-path proof used by the follower, metrics, scripts, and spatial planned-route overlays. The staged search carries incoming edge, direction, and lane across ordered waypoints. A fixed road waypoint can only be satisfied in its selected lane; an opposing lane at the same centerline fraction is a different state and requires a legal detour. Version 7 also treats a junction movement with no lane-to-lane connector as illegal, matching the editor's movement matrix.
- Traversal records contain physical `fromLaneIndex` / `toLaneIndex` assignments plus lane-center `fromSubnode` / `toSubnode` road-arm boundaries; version 7 adds the stable `fromLaneId` / `toLaneId` (and subnode `laneId`) so explicit asymmetric lanes with unequal widths keep their identity when neighbours are inserted or removed. Intersection connectors are sampled canonically between those subnodes. Same-direction lane changes use eight smoothstep samples and never cross an opposing-flow divider.
