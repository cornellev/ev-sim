...
On Map, the Select tool click-selects; a drag (node, prop, asset, road, or knot/handle) starts only after the pointer moves 4px, and the gesture delta is measured from the pointer-down world point so a click on a large GLB footprint does not jump the origin to the cursor. Empty space and lane picks use the same 4px pending-pan gate. While Map is showing, Google tiles stay hidden and `SimulationEngine.sceneRenderEnabled` is false so the covered WebGL scene (including a loaded GLB) is not drawn every frame. Pan/zoom and road-pen cursor updates do not re-render the toolbar or side panes.
