...
test("workspace switcher reaches every workspace at laptop height", async ({ page }) => {
    ...
    const dialog = page.getByRole("dialog", { name: "Workspaces" });
    await expect(dialog).toBeVisible();
    await expect(dialog.getByRole("button")).toHaveCount(14);
    ...
    for (const label of ["Environment editor", "Run configuration", "Scenarios", "Experiment suite", "Headless runs", "Vehicle editor", "Scripting canvas", "Bindings", "Replay", "Visual lab", "Logs", "Analysis"]) {
