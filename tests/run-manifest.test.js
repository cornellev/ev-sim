...
test("run manifest validation rejects future versions and duplicate stable ids", () => {
    assert.throws(() => normalizeRunManifest({ kind: "cev-sim.run-manifest", version: 2 }), /version 2/);
    const manifest = createDefaultRunManifest();
