...
test("canonical numbers absorb sub-picometer float noise across hashers", () => {
    const value = 42.123456789012;
    const perturbed = value + 1e-13;
    ...
    assert.equal(canonicalStringify({ x: value }), canonicalStringify({ x: perturbed }));
    assert.equal(canonicalSimulationStringify({ x: Math.PI }), '{"x":3.14159265359}');
});
